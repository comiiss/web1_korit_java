package _08_Array;

public class Array1 {
    public static void main(String[] args) {
        //배열(Array): 동일한 자료형의 대량 데이터를 관리하는 저장 자료형

        //배열 - 참조 자료형
        //new - heap 메모리 주소를 할당받아 오는 문법
        //배열 변수 이름을 복수형으로 쓰는 건 관례
        int[] scores = new int[5]; //int 5개 들어갈 배열
        //각 배열의 자리는 번호(index)로 지정되어 있음
        scores[0] = 90;
        scores[1] = 80;
        scores[2] = 70;
        scores[3] = 60;
        scores[4] = 99;

        System.out.println(scores); //출력시? 주소값 출력됨
        System.out.println(scores[0]); //scores의 0번칸 조회
        System.out.println(scores[1]);

        //간단하게 배열 생성/초기화 한 번에
        int[] scores2 = new int[]{90, 80, 70, 60, 99};
        int[] scores3 = {90, 80, 70, 60, 99};

        //배열은 new 될 때, 용량이 정해지고, 변경할 수 없다
        //더 큰 용량이 필요하다 - 새로운 배열을 만들어서 복사

        //배열의 길이를 저장하고 있음 .length
        System.out.println(scores3.length);

        //배열과 for문
        for (int i = 0; i < scores3.length; i++) {
            System.out.println(scores3[i]);
        }

        //자료형[] - 자료형을 저장하는 배열 선언
        String[] names = {"박하나", "박둘", "박셋", "박넷", "박다섯"};
        //1. names 배열로 접근해서 박하나, 박넷을 출력해 주세요
        //2. 박다섯을 박오로 수정해 주세요
        //3. names 배열에 있는 모든 이름을 순서대로 출력해 주세요

        System.out.println(names[0]);
        System.out.println(names[3]);

        names[4] = "박오";

        for (int i = 0; i < names.length; i++) {
            String name = names[i]; //i번째 칸 꺼내서
            System.out.println(name); //출력
        }

        //향상된 for문
        //항상 배열의 길이 만큼 순회
        //개발자들은 인덱스로 데이터에 접근하는 것을 막는다.
        //(ex) names[1]
        for (String name : names) {
            System.out.println(name);

            //풀네임만 조회 가능
        }
        int[] nums = {1, 2, 3, 4, 5, 6, 7, 8};
        //nums를 for문으로 순회하면서 짝수만 출력해 주세요

        for(int i = 0; i < nums.length; i++) {
            int num = nums[i];
            if(num % 2 == 0) {
                System.out.println(num);

            }
        }
        for(int num : nums) {
            if (num % 2 == 0) {
                System.out.println(num);
            }

            //String은 참조 자료형 --> char[]

            String str = "My name is Son";
            //배열처럼 길이가 존재, index도 존재한다
            System.out.println(str.length());

            //String의 여러 기능들

            //특정 문자열의 포함 여부 - true/false
            System.out.println(str.contains("Son")); //true

            //특정 문자열이 몇 번째 index에서 시작하는가?
            System.out.println(str.indexOf("name")); //띄어쓰기도 문자다

            //부분문자열 추출 가능
            System.out.println(str.substring(3)); //3~끝까지
            System.out.println(str.substring(3, 8)); //3번~8번
            //참고로 8번 "미만"이다

            String pn = "991122-1234567";
            String birthDate;
            String genderCode;

            //방법1
            birthDate = pn.substring(0, 6);
            System.out.println(birthDate);

            //방법2
            int dashIdx = pn.indexOf("-");
            birthDate = pn.substring(0, dashIdx);
            //0부터 - 직전까지 잘라와라
            System.out.println(birthDate);

            genderCode = pn.substring(dashIdx + 1, dashIdx + 2);
            System.out.println(genderCode);

            String[] names2 = {"김아무개", "박혁거세", "페이커", "을지문덕", "쵸비"};

            //names2를 for문으로 순회하면서 3글자를 초과하는 이름만 출력

            for (String name : names2) {
                if(name.length() > 3) {
                    System.out.println(name);
                }
            }




        }
    }


}


